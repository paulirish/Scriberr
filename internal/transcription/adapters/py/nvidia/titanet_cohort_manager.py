...
def refresh_cohort(
    qdrant_host: str, imposter_collection: str, cohort_collection: str, sample_size: int
):
    """
    ...
    logger.info(
        f"Attempting to refresh cohort '{cohort_collection}' from '{imposter_collection}'"
    )

    try:
        # 1. Get total number of imposter candidates
        imposter_collection_info = client.get_collection(
        ...
        if total_imposters == 0:
            logger.warning(
                "No imposter candidates available to build a cohort. Exiting."
            )
            return
        ...
        client.recreate_collection(
            ...
            vectors_config=qmodels.VectorParams(
                size=192, distance=qmodels.Distance.COSINE
            ),
        )

        # Prepare points for upsertion with new UUIDs for the cohort collection
        new_cohort_points = [
            qmodels.PointStruct(id=str(uuid.uuid4()), vector=point.vector)
            for point in sampled_points
        ]

        client.upsert(
            collection_name=cohort_collection, points=new_cohort_points, wait=True
        )

        logger.info(
            f"Successfully refreshed cohort '{cohort_collection}' with {len(new_cohort_points)} vectors."
        )
        ...
        if "not found" in str(e).lower():
            logger.error(
                f"Collection '{imposter_collection}' not found. Please ensure it exists and has data."
            )
        sys.exit(1)
