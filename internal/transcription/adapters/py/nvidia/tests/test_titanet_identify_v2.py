...
class TestIdentifier(unittest.TestCase):
    ...
    @patch(
        "titanet_identify_v2.os.path.getctime", return_value=12345.0
    )  # Mock getctime
    @patch("titanet_identify_v2.EncDecSpeakerLabelModel")
    @patch("titanet_identify_v2.QdrantClient")
    @patch("titanet_identify_v2.sf")
    def test_enroll_new_speaker(
        self, mock_sf, MockQdrantClient, MockNeMoModel, mock_getctime
    ):  # Added mock_getctime
        ...
        mock_model.side_effect = [
            (None, np.array([[0.1] * 192]))
            for _ in range(10)  # For speaker_0
        ] + [
            (None, np.array([[0.2] * 192]))
            for _ in range(10)  # For speaker_1
        ]
        ...
        imposter_collection_call = any(
            call.kwargs["collection_name"] == "imposter_candidates"
    ...
    @patch("titanet_identify_v2.QdrantClient")
    @patch("titanet_identify_v2.sf")
    def test_match_and_update_speaker(
        self, mock_sf, MockQdrantClient, MockNeMoModel, mock_getctime
    ):  # Added mock_getctime
        ...
        mock_client.search.side_effect = [
            [
                MockPoint(id="123", score=0.8, vector=existing_vector.tolist())
            ],  # speaker_0 matches
            [],  # speaker_1 is new
        ]

        mock_model = MockNeMoModel.restore_from.return_value
        # Use side_effect on the callable mock (mock_model) to return distinct embeddings for each segment
        mock_model.side_effect = [
            (None, np.array([[0.1] * 192]))
            for _ in range(10)  # For speaker_0
        ] + [
        ...
        self.assertAlmostEqual(
            updated_point.vector[0] / np.linalg.norm(updated_point.vector),
            0.1375 / np.linalg.norm([0.1375] * 192),
            places=4,
        )

    @patch("titanet_identify_v2.np.mean")  # Patch np.mean directly
    @patch("titanet_identify_v2.np.std")  # Patch np.std directly
    @patch(
        "titanet_identify_v2.os.path.getctime", return_value=12345.0
    )  # Mock getctime
    @patch("titanet_identify_v2.EncDecSpeakerLabelModel")
    ...
    def test_s_norm_logic(
        ...
        mock_np_std,
        mock_np_mean,
    ):  # Reorder args to match decorators
        mock_client = MockQdrantClient.return_value

        # Mock cohort
        cohort_vectors = [
            MagicMock(vector=np.random.rand(192).tolist()) for _ in range(10)
        ]
        mock_client.get_collection.return_value = MagicMock(
            vectors_count=10
        )  # Ensure vectors_count returns an int
        ...
        # Expected upsert calls: 1 for speaker_0 update, 1 for speaker_1 enrollment, 1 for speaker_1 imposter. Total 3.
