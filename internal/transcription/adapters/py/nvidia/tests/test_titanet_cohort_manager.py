...
class TestCohortManager(unittest.TestCase):
    ...
    def test_refresh_cohort_success(self, MockQdrantClient):
        ...
        mock_imposter_points = [
            MagicMock(vector=[float(i)] * 192) for i in range(10)
        ]
        ...
        mock_client_instance.get_collection.assert_called_once_with(collection_name="imposters")
        ...
        self.assertEqual(kwargs.get("collection_name"), "cohort")
        self.assertEqual(len(kwargs.get("points")), 5) # Check if the sample size was respected
