...
def identify_speakers(
    ...
    qdrant_host: str = "qdrant",
        ...
        try:
            cohort_collection_info = client.get_collection(collection_name=cohort_collection_name)
            ...
            if total_cohort_vectors > 0:
                ...
                logger.warning("S-Norm cohort is empty. Score normalization will be skipped.")
        except Exception as e:
            cohort_embeddings = None
            logger.warning(f"Could not load S-Norm cohort ('{cohort_collection_name}'). Score normalization will be skipped. Error: {e}")
        ...
        for local_spk, indices in local_speakers.items():
            ...
            if cohort_embeddings is not None and len(cohort_embeddings) > 0:
                ...
                if cohort_std > 1e-6: # Avoid division by zero
                    ...
                    logger.info(f"S-Norm: raw_score={best_match_score:.4f}, cohort_mean={cohort_mean:.4f}, cohort_std={cohort_std:.4f} -> norm_score={norm_score:.4f}")
                    final_score = norm_score
                else:
                     logger.warning("Cohort standard deviation is near zero. Skipping normalization.")
            
            decision_metric = final_score
            decision_threshold = norm_threshold if cohort_embeddings is not None else threshold
    
            if decision_metric >= decision_threshold:
                # High confidence match
                best_match = search_result[0]
                global_id = best_match.payload.get("name", "Unknown")
                logger.info(f"Matched {local_spk} to {global_id} (score: {final_score:.4f})")
                ...
                logger.info(f"Enrolling {local_spk} as new speaker {human_name} (raw score: {best_match_score:.4f} < {threshold_new})")
                client.upsert(
                    collection_name=collection_name,
                ...
                logger.info(f"Ambiguous match for {local_spk} (raw score: {best_match_score:.4f}). Enrolling as new temporary speaker.")
                new_id = str(uuid.uuid4())
                human_name = f"Speaker-{new_id[:8]}"
                client.upsert(
                    collection_name=collection_name,
